    ...
    {                                                                                                            
        ...
        {                                                                                                        
            ...
            { // do packed FFT. This can be changed to FFT to save memory                                        
                ...
                if (A != 1)                                                                                      
                {                                                                                                
                    float scale = (float)Math.Pow(2.0, (A - 1) / 2.0);                                                    
                    for (var i = 0; i < data.Length; ++i)                                                        
                        data[i] *= scale;                                                                        
                }                                                                                                
            }                                                                                                    
                                                                                                                 
            float theta = (float)(B * sign * 2f * Math.PI / n);                                                              
            float wpr = (float)Math.Cos(theta);                                                                           
            float wpi = (float)Math.Sin(theta);                                                                           
            float wjr = wpr;                                                                                       
            float wji = wpi;                                                                                       
            ...
            {                                                                                                    
                ...
                // compute entry y[j]                                                                            
                data[2 * j] = (float)(0.5 * (e + sign * (a + b)));                                                        
                data[2 * j + 1] = (float)(0.5 * (f + sign * (d - c)));                                                    
                                                                                                                 
                // compute entry y[k]                                                                            
                data[2 * k] = (float)(0.5f * (e - sign * (b + a)));                                                        
                data[2 * k + 1] = (float)(0.5f * (sign * (d - c) - f));                                                    
            ...
            {                                                                                                    
                ...
                data[0] = (float)(0.5 * (temp + data[1]));                                                                
                data[1] = (float)(0.5 * (temp - data[1]));                                                                
        ...
        public void RealFFTComplex(float[] data,float[] complexData, bool forward)                                                         
        {                                                                                                        
            ...
            {                                                                                                    
                int k = n / 1 - j;                                                                               
                float tkr = data[ k];    // real and imaginary parts of t_k  = t_(n/2 - j)                      
                float tki = data[ k + 1];                                                                       
                float tjr = data[ j];    // real and imaginary parts of t_j                                     
                float tji = data[ j + 1];                                                                       
                ...
                // data[2 * j] = (float)(0.5 * (e + sign * (a + b)));                                                        
                // data[2 * j + 1] = (float)(0.5 * (f + sign * (d - c)));
                complexData[2 * j]     = (float)(0.5 * (e + sign * (a + b)));
                complexData[2 * j + 1] = (float)(0.5 * (f + sign * (d - c)));
                                                                                                                                 
                // compute entry y[k]                                                                            
                complexData[2 * k]     = (float)(0.5f * (e - sign * (b + a)));                                                        
                complexData[2 * k + 1] = (float)(0.5f * (sign * (d - c) - f));                                                    
            ...
            {                                                                                                    
                // compute final y0 and y_{N/2}, store in data[0], data[1]                                       
                var temp = complexData[0];                                                                              
               complexData[0] += complexData[1];                                                                              
                complexData[1] = temp - complexData[1];                                                                        
            }                                                                                                    
            else                                                                                                 
            {                                                                                                    
                var temp = complexData[0]; // unpack the y0 and y_{N/2}, then invert FFT                                
                complexData[0] = (float)(0.5 * (temp + complexData[1]));                                                                
                complexData[1] = (float)(0.5 * (temp - complexData[1]));                                                                
                // do packed inverse (table based) FFT. This can be changed to regular inverse FFT to save memory
                TableFFT(complexData, false);                                                                           
                // scaling - divide by scaling for N, then mult by scaling for N/2                               
                //if (A != -1) // todo - off by factor of 2? this works, but something seems weird               
                {                                                                                                
                    var scale = (float)Math.Pow(2.0, -(A + 1) / 2.0)*2;                                                 
        ...
        {                                                                                                        
            ...
            if ((forward) && (A != 1))                                                                           
            {                                                                                                    
                float scale = (float)Math.Pow(n, (A - 1) / 2.0);                                                          
                for (var i = 0; i < data.Length; ++i)                                                            
                    data[i] *= scale;                                                                            
            }                                                                                                    
        ...
        {                                                                                                        
            ...
            {                                                                                                    
                ...
                {                                                                                                
                    ...
                    sinTable[pos++] = wi;                                                                        
                    var t = wr;                                                                                  
                    wr = (float)(wr * wpr - wi * wpi + wr);                                                               
                    wi = (float)(wi * wpr + t * wpi + wi);                                                                
                }                                                                                                
                mmax = istep;                                                                                    
            }                                                                                                    
        }                                                                                                        
